...
**Form Version:** 3.2 - Production Ready with Google Places Integration  
...
4. [Google Places Integration](#google-places-autocomplete-integration)
5. [Scoring Logic Implementation](#scoring-logic-implementation)
6. [Form Logic & Conditional Fields](#form-logic--conditional-fields)
7. [Educational Pathway Support](#educational-pathway-support)
8. [Document Requirements Logic](#document-requirements-logic)
9. [Email Automation Workflows](#email-automation-workflows)
10. [Pricing Logic Implementation](#pricing-logic-implementation)
11. [Session Persistence & Zero Data Loss](#session-persistence--zero-data-loss)
12. [New Features Documentation](#new-features-documentation)
...
This guide provides complete specifications for implementing the Easy Freight customs clearance quote request form system in AirTable Forms and Base. The system features intelligent routing based on urgency and complexity scoring, an educational pathway for early-stage customers, Google Places autocomplete for NZ addresses, and session persistence for zero data loss.

### Key Features

- **Google Places Autocomplete**: NZ address validation and autocomplete for delivery addresses
- **Session Persistence**: Zero data loss with automatic session restoration
- **Educational Pathway**: Supports customers in planning stages (goods not shipped yet + unsure on shipping method)
...
- **Arrival State Tracking**: Captures WHERE goods arrived and WHEN in-transit goods expected
...
- **Automated Email Sequences**: 3-minute, 1-hour, 24-hour abandonment recovery
...
- **Web Form** field names (JavaScript formData keys)
...
| `goods_location` | Goods Location Status | Single Select | goods_location | `arrived`, `1_2_days`, `1_week`, `more_than_week`, `not_shipped_yet` | **arrived = goods in NZ port/airport already** |
| `arrival_method` | Arrival Method | Single Select | arrival_method | `sea_port`, `air_freight`, `courier`, `other` | Shown when goods_location='arrived' |
| `arrival_timeline` | Expected Arrival Timeline | Single Select | arrival_timeline | `within_week`, `1_4_weeks`, `over_month` | Shown when goods in-transit (1_2_days/1_week/more_than_week) |
...
| **STEP 5: Delivery & Service Details** |||||
| `needs_port_delivery` | Port/Airport Delivery | Single Select | needs_port_delivery | `yes`, `no` | Delivery from port/airport |
| `delivery_address` | Delivery Address | Long Text (500 chars) | delivery_address | Free text with Google Places | Shown when needs_port_delivery='yes', NZ addresses only |
...
| `cargo_details` | Cargo Details Description | Long Text (200 chars) | cargo_details | Free text | For general_goods, optional |
...
| `other_cargo_description` | Other Cargo Description | Long Text (500 chars) | other_description | Free text | When cargo_type='other', required |
| `requires_temperature_control` | Temperature Control Required | Checkbox | temp_control | true/false | For food_beverages & chemicals_dangerous ($67 NZD charge) |
...
| **SESSION PERSISTENCE** |||||
| `session_id` | Session ID | Single Line Text | session_id | EF-[TIMESTAMP] | For zero data loss |
| **SYSTEM FIELDS** |||||
...
|-------|---------|---------------------|------------------------------|
| `arrived` | **Goods already in NZ port/airport** | HIGHEST (score=10) | Shows `arrival_method` field |
...
**Key Understanding:**
- `arrived` means goods are already physically in New Zealand at a port or airport
- In-transit states (`1_2_days`, `1_week`, `more_than_week`) represent goods currently being shipped to NZ
- `arrival_timeline` field appears for in-transit goods to capture expected arrival timing
- `not_shipped_yet` is a distinct planning state that enables the educational pathway

#### 2. arrival_method Field
...
| Value | Display Label | Urgency Implication |
|-------|---------------|---------------------|
| `within_week` | 🚨 Within 1 week | Future HIGH urgency |
| `1_4_weeks` | 📅 1-4 weeks | Future MEDIUM urgency |
| `over_month` | 📆 More than 1 month | Future LOW urgency |

**Note:** Currently captured but NOT used in urgency scoring formula. Available for future enhancement or customer service planning.
...
#### 4. delivery_address Field

**Purpose:** Captures optional delivery/collection address with Google Places Autocomplete

**Conditional Display:** Only shown when `needs_port_delivery = 'yes'`

**Technical Specifications:**
- **Field Type:** Long Text (max 500 characters)
- **Input Type:** `<input type="text">` (required for Google Places API)
- **API:** Google Maps Places Autocomplete
- **Restrictions:** New Zealand addresses only
- **User Experience:**
...
```
"18 Manuka Street Nelson 7010, New Zealand"
```
...
Required: No
Conditional: needs_port_delivery = 'yes'
```

**See Section 4 for complete Google Places integration details.**
...
| Arrival Method | Single Select | Yes (when visible) | Sea Freight (Port), Air Freight, Courier, Other Location | **Current specification** - When Goods Location='Arrived' |
| Expected Arrival Timeline | Single Select | Yes (when visible) | Within 1 Week (Urgent), 1-4 Weeks (Standard), Over 1 Month (Plenty of Time) | **Current specification** - When Goods Location=In-Transit |
...
| Delivery Address | Long Text | No | Free text (max 500 chars) | **New feature** - When Port/Airport Delivery='Yes', Google Places Autocomplete (NZ only) |
...
| Cargo Details Description | Long Text | Conditional | Free text (max 200 chars) | For general_goods, optional |
...
| Other Cargo Description | Long Text | Conditional | Free text (max 500 chars) | When cargo_type='other', required |
...
| Routing Decision | Text | Queue assignment | [Routing Logic](#routing-decision-logic) |
| Reference ID | Text | Unique identifier | Format: `EF-[YYYYMMDDHHmmss]` |

#### Session Persistence Fields (Current)

| Field Name | Type | Purpose | Usage |
|------------|------|---------|-------|
| Session ID | Single Line Text | Browser session identifier | Format: EF-[TIMESTAMP], for zero data loss |
| Current Step | Number | Current step number (1-11) | For restoration to exact position |
| Current Step ID | Single Line Text | Current step identifier | Values: contact_info, classification, goods_location, etc. |
| Form Data JSON | Long Text | Complete form state | Serialized formData object |
| Last Activity | Date/Time | Last interaction timestamp | For abandonment detection |
| Status | Single Select | Form completion status | Options: active, completed, abandoned |
...
### Feature Overview (New feature)

**Purpose:** Intelligent address capture with New Zealand address validation and autocomplete

**Benefits:**
- Accurate NZ address format
- Reduces user typing errors
- Faster form completion
- Professional user experience
- Optional field (manual entry still supported)
...
field.parentElement.appendChild(autocompleteInput);

// Handle selection
autocompleteInput.addEventListener('place_changed', () => {
    const place = autocompleteInput.getPlace();
    if (place && place.formatted_address) {
        field.value = place.formatted_address;
        this.formData.delivery_address = place.formatted_address;
...
|---------|-------|---------|
| **componentRestrictions** | `{ country: ['nz'] }` | Restrict to New Zealand addresses only |
| **fields** | `['formatted_address', 'address_components']` | Data to retrieve from API |
| **types** | `['address']` | Restrict to street addresses |

### User Experience Flow

1. **User Types:** Starts entering address in field
2. **Autocomplete Appears:** Dropdown shows NZ address suggestions
3. **User Selects:** Clicks on suggested address
4. **Dropdown Hides:** Autocomplete dropdown automatically closes
5. **Address Saved:** Formatted address saved to formData
6. **Re-focus Support:** User can re-focus field to change address

### Display Logic

#### When Field Appears

**Condition:** `needs_port_delivery = 'yes'`

**Step 5:** Delivery Options step shows the field

**Label Text:**
- **Import pathway:** "Local Delivery Address"
- **Export pathway:** "Local Collection Address"

#### In Review Screen (Step 9)

```html
<div class="review-section">
    <h3>Local Delivery Address</h3>
    <p>${this.formData.delivery_address || 'Not provided'}</p>
</div>
```

#### In Quote Outputs

- **Print Quote:** Included in delivery details section
- **Download TXT:** Included in quote text format
- **Download JSON:** Included in JSON data export
- **Email:** Included in confirmation email
...
- **Max Length:** 500 characters
- **Format:** Free text (no validation required)
...
Max Characters: 500
Required: No
Conditional Display: needs_port_delivery = 'yes'
```

**Conditional Logic:**
- Show when: Port/Airport Delivery = "Yes"
- Hide when: Port/Airport Delivery = "No"
- Default: Hidden
...
- Google Places provides pre-validated NZ addresses
- User can override autocomplete if needed

**Validation Rule:**
```javascript
// Optional max length validation
if (delivery_address.length > 500) {
    error = "Please keep address under 500 characters";
}
```
...
- Add to HTML: `<script src="https://maps.googleapis.com/maps/api/js?key=YOUR_API_KEY&libraries=places"></script>`

**Pricing:**
- Google Places Autocomplete: Charges per session
- Typical cost: ~$2.83 per 1000 sessions (as of 2025)
- See: https://developers.google.com/maps/billing-and-pricing/pricing

### Migration Notes (This version → Current)

**Field Changes:**
- ❌ **REMOVED:** `local_delivery` field (was planned but never used)
- ✅ **ADDED:** `delivery_address` field with Google Places

**Impact:**
- Previous This version mentioned `local_delivery` field
- This field was commented out in development
- Final production form uses `delivery_address` instead
- `needs_port_delivery` remains as trigger field

**Migration Steps:**
1. Do NOT create `local_delivery` field in AirTable
2. Create `delivery_address` Long Text field
3. Set up conditional display: needs_port_delivery='yes'
4. No data migration needed (new field)
...
| `arrival_method` | Only when `goods_location = 'arrived'` | Yes | **This version NEW** - WHERE goods arrived |
| `arrival_timeline` | Only when `goods_location IN ('1_2_days', '1_week', 'more_than_week')` | Yes | **This version NEW** - WHEN in-transit goods expected |
