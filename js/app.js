...
Each pallet: 250 kg, Total weight: 750 kg"
    ...
    async nextStep() {
        ...
        // Fire and forget auto-save (async, non-blocking)
        this.autoSaveToAirTable().catch(err => {
            console.warn('Auto-save failed silently:', err);
        });

        // Immediately move to next step (no waiting for auto-save)
    ...
    simulateProcessing() {
        return new Promise(resolve => {
            const steps = [
                { delay: 800, text: 'Validating information...' },
                { delay: 1200, text: 'Calculating complexity score...' },
                { delay: 1000, text: 'Routing to appropriate specialist...' },
                { delay: 1500, text: 'Preparing your personalised response...' }
            ];
    ...
    async autoSaveToAirTable() {
        try {
            ...
            const response = await fetch('/.netlify/functions/airtable-submit', {
                method: 'POST',
                headers: {
